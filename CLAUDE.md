...
- **TR069 massal (per-OLT)** — tombol "TR069 Massal" di halaman GPON Port (`Pages/SmartOlt/GponPorts.vue` + `Components/SmartOlt/Tr069BulkModal.vue`) mengaktifkan TR069/ACS di **semua ONU** satu OLT ZTE. `app/Services/ZteTr069BulkService.php` membaca running-config tiap ONU per port (1 sesi telnet/port via `ZteOnuRunningConfigService::fetchMany`), **men-skip** ONU yang TR069-nya sudah unlock & ACS-nya (url+username) sudah mengarah ke target, lalu menulis `tr069-mgmt 1 state unlock` + acs line ke sisanya (1 sesi tulis/port). Dua fase lewat flag `execute`: dry-run (pindai saja) lalu eksekusi. Jalan di queued job (`Tr069BulkConfigJob` + tabel `tr069_bulk_tasks`, di-poll). Default ACS dari `config('services.acs')` (`ACS_URL`/`ACS_USERNAME`/`ACS_PASSWORD`, default `http://acs.bmkv.net:7547`/`cms`/`kusuma123!`). Routes `smartolt.tr069-bulk` (POST) + `smartolt.tr069-bulk.status` (GET), gated `supports_cli_onu_configure`.
