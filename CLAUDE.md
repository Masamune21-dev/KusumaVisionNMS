...
- **Scheduled polling DOES use a Go engine** (`bin/kv-snmp-poller`, source `cmd/kv-snmp-poller/`) — see Architecture. On-demand actions (manual refresh, provisioning, telnet, per-port RX) are synchronous PHP in controllers/services — **except batch copy-ONU-to-port**, which runs in a queued job (`app/Jobs/CopyOnusToPortJob.php` + `copy_onu_tasks` table, polled for progress).
