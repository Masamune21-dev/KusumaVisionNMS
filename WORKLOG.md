...
- **Blocker lingkungan (RESOLVED)**: `bootstrap/cache/routes-v7.php` sempat basi (dimiliki `www-data`, tertanggal sebelum sesi ini) — rute baru 404 sampai ownership `bootstrap/cache/` dipindah ke `netlinkdo:www-data` (2775) dan `route:cache` dijalankan ulang. Terverifikasi: `route:list` menampilkan `zones.*`/`onu-zone.assign`/`api.zones.index`, dan `route('zones.index')` dkk. resolve ke URL absolut yang benar.
- **`phpunit` kini terpasang** (`composer install` — sempat `--no-dev` di server ini) dan **suite jalan penuh setelah `php artisan config:clear`**: `bootstrap/cache/config.php` yang basi membuat `phpunit.xml` punya `DB_CONNECTION=sqlite`/`DB_DATABASE=:memory:` tapi **diabaikan** (config cache selalu menang atas env runtime) — run pertama `ZoneTest` sempat benar-benar menyentuh Postgres produksi (`kusumavision_nms`) dan gagal karena bentrok unique constraint dgn 11 zona asli; **diverifikasi tak ada data produksi yang rusak** (transaksi per-test Laravel roll-back otomatis meski exception). Setelah `config:clear` (aman — `.env` `netlinkdo:www-data 640`, terbaca proses `www-data`) + validasi sanity (`config('database.default') === 'sqlite'`), suite penuh (400 test) jalan bersih terhadap sqlite in-memory; config dicache ulang (`config:cache`) setelahnya untuk kembali ke steady-state produksi.
- **Bug nyata ketemu & diperbaiki**: migrasi awal `onu_zone_links.zone_id` lupa `->nullable()` padahal FK-nya `->nullOnDelete()` — hapus zona TANPA reassign selalu gagal (violated NOT NULL saat SET NULL dieksekusi), padahal itu perilaku yang diminta ("Sin zona" untuk ONU yang tak direassign). Migrasi susulan `2026_07_24_000003_make_zone_id_nullable_on_onu_zone_links_table.php` (`->nullable()->change()`) sudah dijalankan di produksi (data existing aman, 11 zona/0 link tak berubah).
- **Regresi test terdeteksi & diperbaiki**: `zone_id` wajib di validasi registrasi (mode dasar & Lanjutan) mematahkan 5 test lama yang belum kirim `zone_id` (`SmartOltAdvancedRegisterTest`, `Api/ApiV1WriteTest`) — payload helper masing-masing ditambah `zone_id` (via `Zone::firstOrCreate`).
- **1 kegagalan pre-existing, di luar scope zonas** — `ApiV1WriteTest::test_refresh_port_non_zte_queries_driver`: controller `OnuActionController::refreshPort` cabang ZTE/non-ZTE lewat `SmartOltSupport::isNonZte()` (static), bukan `$resolver->isNonZte()` yang di-mock test — mock jadi mati, request nyasar ke jalur SNMP asli tak ter-mock. Tak disentuh di sini (di luar scope "zonas"); di-flag sebagai task terpisah.
- **Hasil akhir**: 400 test, 1 kegagalan (pre-existing, tak terkait), 0 regresi dari fitur zonas.
