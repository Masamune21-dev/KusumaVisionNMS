...
### Perangkat Aktif & Jejak Audit (lanjutan SSO)

IdP mendapat halaman "Perangkat Aktif" dan jejak audit terpusat. Dampaknya ke NMS hanya
satu berkas: `app/Services/Sso/HubSession.php`.

Changed:
- `HubSession::touch()` kini menyegarkan TIGA kunci (`sid:{sid}`, `:meta`, `:apps`) dalam
  satu pipeline Redis, bukan hanya kunci utamanya. Tanpa ini kunci metadata kedaluwarsa
  duluan dan halaman Perangkat Aktif kehilangan keterangan perangkat justru saat sesinya
  masih dipakai. Tetap satu round-trip per request.

Notes:
- Metadata sengaja ditaruh di kunci TERPISAH, bukan mengubah `sid:{sid}` jadi hash —
  supaya sesi yang sudah terbit sebelum fitur ini ada tetap sah (tidak ada WRONGTYPE).
- Daftar app yang disentuh satu sesi dicatat oleh IdP saat menerbitkan code, BUKAN oleh
  app klien, supaya jalur request app klien tidak bertambah satu penulisan pun.
- Ganti password di IdP kini mencabut seluruh sesi lain milik orang itu — termasuk sesi
  NMS-nya. Ini disengaja.

---

### Single Sign-On terpusat: NMS berhenti memeriksa password sendiri

Autentikasi ketiga app Triad dipindah ke Identity Provider baru `sso.kusumavision.net`
(`/var/www/KusumaVisionSSO`, Laravel 12 + Inertia). NMS kini memakai **Authorization Code
Flow + PKCE S256**: middleware `auth` mengarahkan ke `/sso/redirect`, IdP memantulkan
kembali dengan authorization code, lalu code ditukar jadi klaim identitas lewat panggilan
back-channel server-ke-server. Karena IdP mengenali sesi yang sudah ada, membuka NMS
setelah login di MikroTik/Billing tidak memunculkan form apa pun.

Kenapa bukan berbagi cookie antar-subdomain: `APP_KEY` ketiga app berbeda dan session
store-nya terpisah (Redis DB 1/3/5), jadi cookie NMS secara matematis tak akan pernah bisa
didekripsi app lain.

Created:
- `config/sso.php`, `app/Support/Pkce.php`
- `app/Services/Sso/{SsoClient,HubSession}.php`
- `app/Http/Controllers/Auth/SsoController.php` (redirect / callback / logout / login darurat)
- `app/Http/Middleware/EnsureSsoSessionAlive.php`
- `app/Console/Commands/SsoEmergencyLogin.php`
- `resources/js/Components/Shell/AppSwitcher.vue`
- migrasi `add_sso_user_id_to_users_table`
- `tests/Feature/Sso/{SsoClientTest,SessionRevocationTest}.php`, `tests/Concerns/SimulatesSso.php`

Changed:
- `routes/auth.php` — form login/registrasi/reset password DIHAPUS; rute bernama `login`
  kini menunjuk `/sso/redirect` supaya seluruh `route('login')` dan redirect bawaan
  Laravel tetap bekerja tanpa diubah satu per satu. `/login` disisakan sebagai pengalihan
  untuk bookmark lama.
- `bootstrap/app.php` — `EnsureSsoSessionAlive` dipasang SEBELUM `HandleInertiaRequests`
  supaya sesi yang sudah dicabut tak sempat ikut membangun props halaman.
- `config/database.php` — koneksi Redis `sso` (DB 6, prefix tetap `kvsso:`).
- `HandleInertiaRequests` — shared prop `ecosystem` (dibaca dari klaim `apps` yang
  tersimpan di session, TANPA panggilan HTTP per-request) + `ssoIssuer`.
- `AuthenticatedLayout.vue` — App Switcher di header desktop & mobile.
- `UserController` — pembuatan akun ditolak (identitas milik IdP); `update()` kini hanya
  menyimpan penugasan OLT. Nama/email/password/role diabaikan karena ditimpa ulang dari
  klaim SSO tiap login.
- Halaman Profil jadi baca-saja + tautan ke pengaturan akun di IdP.
- `tests/TestCase.php` — `actingAs()` ikut menerbitkan sid hub (ke Redis DB uji), kalau
  tidak seluruh test yang memakai `actingAs()` akan menerima redirect ke login.

Notes:
- **Logout wajib `Inertia::location()`, bukan `redirect()->away()`.** Tombol logout adalah
  `<Link method="post">` alias XHR; balasan 302 lintas-domain akan diikuti axios lalu gagal
  (CORS / balasan bukan-Inertia). Ini sempat lolos uji curl karena curl tidak mengirim
  header `X-Inertia`. Berlaku sama untuk `/sso/redirect` dan middleware sesi.
- `adoptGuestLocale()` dipindahkan dari `AuthenticatedSessionController` (dihapus) ke
  `SsoController::callback()` — session app bertahan selama perjalanan ke IdP dan kembali,
  jadi pilihan bahasa tamu di halaman publik tidak hilang.
- Penjaga "admin terakhir tidak boleh diturunkan" pindah ke IdP; role tidak lagi bisa
  diubah dari sini sama sekali.
- Verifikasi: `bash scripts/test.sh` 543 passed / 2895 assertions. Uji ujung-ke-ujung
  lintas-app (login sekali → tiga dashboard, single logout, pencabutan instan) lulus di
  server nyata.

## 2026-08-31

### Perbaikan Tombol Header Halaman App Tidak Bisa Diklik (canvas partikel menelan klik)

Semua tombol di header per-halaman `AuthenticatedLayout` (mis. BACK / EDIT / GPON PORT & ONU /
REGISTRATIONS / CONFIG BACKUP / REFRESH SNMP di Detail OLT) mati total — klik tidak sampai ke
tombol. Penyebabnya latar `ParticleNetwork`, bukan halamannya:

1. Canvas partikel dipasang `!fixed inset-0` di dalam `<main>`. `<main>` memakai `.kv-grid-bg`
   (`position: relative` + `isolation: isolate`) sehingga dicat SETELAH `<header>` slot yang
   static — canvas full-viewport itu menutupi area header.
2. tsParticles menulis inline `pointer-events: initial` ke canvas-nya sendiri saat
   `detectsOn: 'canvas'` + hover aktif (`InteractivityEventListeners` -> `canvas.setPointerEvents`),
   menembus `pointer-events-none` milik wrapper. Terverifikasi di browser: canvas login punya
   `style.pointerEvents = "initial"` walau wrapper `none`; repro `elementFromPoint` di atas tombol
   mengembalikan `CANVAS`, dan `BUTTON` begitu pointer-events canvas dimatikan.

Changed:

- `resources/js/Components/Shell/ParticleNetwork.vue` - prop baru `interactive` (default `true`,
  perilaku Welcome/Guest tak berubah); saat `false` hover `grab` dimatikan dan canvas dipaksa
  `pointer-events: none !important` lewat scoped `:deep(canvas)` (declaration !important dari
  stylesheet mengalahkan inline non-important milik tsParticles).
- `resources/js/Layouts/AuthenticatedLayout.vue` - latar partikel app shell dipasang
  `:interactive="false"` + komentar alasannya.

Notes:

- Terdampak SEMUA halaman app yang memakai slot `header`, bukan cuma Detail OLT.
- Konsekuensi: latar app shell kini murni dekoratif (tanpa efek grab kursor) — sekalian hemat
  CPU. Kalau efeknya mau dikembalikan tanpa memblokir klik, satu-satunya jalan adalah
  `detectsOn: 'window'` (listener di document, canvas tetap pointer-events none) dengan
  konsekuensi mousemove seluruh halaman menghitung ulang link grab.
- `npm run build` dijalankan ulang (aset ber-hash baru); `npm test` 12 passed.
