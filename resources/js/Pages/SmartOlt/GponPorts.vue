            ...
            <div class="w-full space-y-6 px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    ...
                    <div class="rounded-lg border border-sky-200 bg-white p-5 shadow-sm shadow-sky-100/60">
                        ...
                        <p class="mt-3 text-2xl font-bold text-slate-900">{{ onuOnline }}<span class="text-sm font-normal text-slate-500"> / {{ onuTotal }}</span></p>
                    </div>
                    <div class="rounded-lg border border-sky-200 bg-white p-5 shadow-sm shadow-sky-100/60">
                        <p class="text-xs font-medium uppercase tracking-wider text-slate-500">Status SNMP</p>
                        <p class="mt-3 text-2xl font-bold" :class="snapshot.ok ? 'text-emerald-600' : 'text-slate-400'">
                ...
                <div class="overflow-hidden rounded-lg border border-sky-200 bg-white shadow-sm shadow-sky-100/60">
                    <div class="flex flex-col gap-4 border-b border-slate-100 px-4 py-4 sm:px-5 md:flex-row md:items-center md:justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-base font-semibold text-slate-900">GPON Port & ONU</h3>
                        </div>
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                            <input
                                v-model="portSearch"
                                type="search"
                                class="min-h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-sky-400 sm:w-72"
                                placeholder="Cari ONU (SN/Nama)..."
                            />
                            <span class="inline-flex h-8 items-center gap-1.5 rounded-full bg-emerald-50 px-3 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200">
                    ...
                    <div v-if="snapshot.ports.length === 0" class="px-6 py-10 text-center text-sm text-slate-500">
                        Belum ada data port. Jalankan <strong class="text-slate-700">Refresh SNMP</strong>.
                    </div>
                    ...
                    <div v-else class="grid gap-3 p-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                        ...
                        >
                            <div class="font-mono text-sm font-semibold text-slate-800">{{ port.name }}</div>

                            <div class="mt-4 flex items-end justify-between gap-3">
                                <span class="text-xs font-bold"
                                      :class="port.oper_status === 'up' ? 'text-emerald-600' : 'text-slate-400'">
                            ...
                            <div v-if="isSearching && port.matching_onus.length"
                                ...
                                <div v-for="onu in port.matching_onus.slice(0, 3)"
                                    ...
                                    <span class="truncate font-medium text-slate-700">{{ onuSummary(onu) }}</span>
                                    <span class="shrink-0 font-semibold"
                                          :class="onu.online ? 'text-emerald-600' : 'text-slate-400'">
                                        {{ onu.online ? 'ON' : 'OFF' }}
                                    </span>
                                </div>
