...
const form = useForm({
    name: props.olt?.name ?? '',
    // HiOSO satu family (enterprise 25355) — vendor tetap, tak ada pilihan family.
    vendor: props.olt?.vendor ?? props.defaults.vendor ?? 'HiOSO EPON 25355',
    ip: props.olt?.ip ?? '',
...
const submit = () => {
    if (props.olt) {
        form.put(route('hioso-olt.update', props.olt.id), {
            preserveScroll: true,
        });
    ...
    form.post(route('hioso-olt.store'), {
        preserveScroll: true,
    });
};
</script>
