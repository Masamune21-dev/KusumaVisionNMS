...
import {
    Activity, Check, Cloud, Cpu, Eye, EyeOff, Globe, ListChecks, Network,
    Plus, Settings, ShieldCheck, Trash2, X,
} from '@lucide/vue';
import { computed, onMounted, onUnmounted, ref } from 'vue';

// Editor granular ONU yang dipakai bersama oleh halaman Configure ONU (delta)
// dan Register ONU · mode Lanjutan (registrasi penuh). Memutasi objek `config`
// di tempat (reactive, by-reference) — parent yang mengurus preview/apply.
                    ...
                    <div v-for="(row, i) in cfg.vlan_ports" :key="`uv-${i}`" :class="isWide ? 'kv-trow' : 'kv-rowcard'" :style="isWide ? { gridTemplateColumns: cols.uniVlan } : null">
                        ...
                        <div class="kv-cell"><span v-if="!isWide" class="kv-flabel">Mode</span>
                            <select v-model="row.mode" :class="fieldClass">
                                ...
                                <option value="transparent">transparent</option>
                                <option value="na">na (hapus)</option>
                            </select>
                        </div>
