    ...
    <div class="kv-glass-panel flex h-full flex-col">
        <div class="flex items-center justify-between border-b border-white/5 px-5 py-4">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-base font-semibold text-white">Inventory OLT (Ringkasan)</h3>
            </div>
        ...
        <ul v-if="items.length > 0" class="flex-1 divide-y divide-white/5">
            <li v-for="row in items" :key="row.model" class="flex items-center justify-between gap-3 px-5 py-3.5">
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        <p class="truncate text-sm font-semibold text-white">{{ row.model }}</p>
                        <p class="text-xs text-slate-500">{{ row.unit }} Unit</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <span class="kv-pill-success">Up {{ row.up }}</span>
                    <span class="kv-pill-danger">Down {{ row.down }}</span>
                </div>
            </li>
        </ul>
